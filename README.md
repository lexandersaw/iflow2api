...
如果你需要通过代理访问 iFlow API（例如在使用 CC Switch 等工具时），可以配置上游代理。

**问题背景**：某些工具（如 CC Switch）会设置系统代理环境变量，导致 iflow2api 的请求被错误路由，返回 502 错误。

**配置方式**

在 GUI 应用中：点击"应用设置"按钮，找到"代理设置"部分：
- 勾选"启用上游代理"
- 填写代理地址，如 `http://127.0.0.1:7890` 或 `socks5://127.0.0.1:1080`

或编辑配置文件 `~/.iflow2api/config.json`：
...
| 配置项                    | 说明                                                |
| ------------------------- | --------------------------------------------------- |
| `upstream_proxy_enabled` | 是否启用代理，默认 `false`                        |
| `upstream_proxy`         | 代理地址，支持 `http://` 和 `socks5://` 协议 |

> **注意**：默认情况下，iflow2api 不会读取系统代理环境变量（`HTTP_PROXY`/`HTTPS_PROXY`），以避免与本地代理工具冲突。只有当你明确配置代理后，才会使用指定的代理服务器。
